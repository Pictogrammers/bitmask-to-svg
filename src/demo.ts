...
window.addEventListener('load', () => {
  ...
  $grid?.style.setProperty('--grid-size', `${scale - 1}`);
  const $svg = document.getElementById('preview');
  ...
  if ($svg) {
    $path = $svg.querySelector('path') as SVGPathElement;
    $svg.setAttribute('viewBox', `0 0 ${width * scale} ${height * scale}`);
  ...
  $scale?.addEventListener('change', (e: any) => {
  ...
  palettes.forEach((label, color) => {
    ...
    $button.addEventListener('click', (e) => {
      colorIndex = parseInt((e.target as HTMLElement).dataset.color ?? '0', 10);
      buttons.forEach(($btn) => {
